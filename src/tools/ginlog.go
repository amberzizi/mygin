...
func InitLogger(){
	...
	logsetting := GetSetting()//配置文件
	var l = new(zapcore.Level)
	err := l.UnmarshalText([]byte(logsetting.Log.Level))
	if err != nil {
		return
	}
	core := zapcore.NewCore(getEncoder(), getLogWrite(GetSetting().Log.Maxsize,
		GetSetting().Log.Maxbackups,GetSetting().Log.Maxage,false),l)
